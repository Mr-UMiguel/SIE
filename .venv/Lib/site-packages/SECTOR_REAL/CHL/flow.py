...
from SECTOR_REAL.CHL.extract import run
from SECTOR_REAL.CHL.transform  import clean
...
def transform(filename):
    print("""
        Iniciando la transformación del PIB de CHL
    """)
    return clean(filename)


with Flow("Extract 1.01.1 CHL") as flow:
    filename = extract()
